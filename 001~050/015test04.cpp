...
// try to handle 2 equaivalent case but decide to seperate;
  for(;i>=2;i--)
  {
    if(a[i]<0) break;
    if(a[i]==a[i-1])
    {
      maps[a[i]]=maps.at(a[i])-2;
      if(maps.count(-a[i]-a[i])) sol.push_back({-a[i]-a[i],a[i],a[i]});
      maps[a[i]]=maps.at(a[i])+1;
      i--;
    }
    while(true)
    {
      if(a[i]!=a[i-1]) break;
      else{maps[a[i]]=maps.at(a[i])-1;i--;}
    }
    for(int j=i-1;j>=0;j--)
    {
      maps[a[j]]--;
