...
string longestPalindrome(string s){
  ...
  for(i=1;i<s.size()-k;i++)
  {
    ...
    {state=0;if(len>max){max=len;answer=s.substr(k+i-len,len);} len=0;break;}
    else if(state==2&&s[k+i]!=s[k+i-1]&&s[k+i]==s[k+i-len-1]){len=len+2;lenb=1;}
    else if(state==2&&s[k+i]==s[k+i-1]&&s[k+i]==s[k+i-len-1]){len=len+2;lenb++;}
    else if(state==2&&s[k+i]==s[k+i-1]&&s[k+i]!=s[k+i-len-1])
    {state=1;if(len>max){max=len;answer=s.substr(k+i-len,len);} len=lenb+1;lenb=0;}
    else if(state==2&&s[k+i]!=s[k+i-1]&&s[k+i]!=s[k+i-len-1])
    {state=0;if(len>max){max=len;answer=s.substr(k+i-len,len);} len=0;lenb=0;break;}
  }
